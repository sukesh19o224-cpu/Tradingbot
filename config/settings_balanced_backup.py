...
# MOMENTUM Strategy - BALANCED 10-day rotation (institutional standard)
MOMENTUM_CONFIG = {
    'STOP_LOSS': 0.02,  # 2% stop loss (BALANCED - tighter for quick exits)
    'TARGETS': [0.025, 0.04, 0.06],  # 2.5%, 4%, 6% targets (BALANCED - achievable in 5-7 days)
    'MIN_ADX': 20,  # Emerging momentum (catch early trends, institutional standard)
    'MIN_RSI': 50,  # Above 50 (momentum zone)
    'MAX_RSI': 65,  # Below 65 (BALANCED - avoid overbought, still has room to move)
    'MAX_DISTANCE_FROM_MA20': 10.0,  # Not more than 10% above 20-day MA (BALANCED - avoid extended)
    'MIN_VOLUME_RATIO': 1.5,  # Minimum 1.5x volume (BALANCED - institutional confirmation)
    'REQUIRE_ABOVE_MA50': True,  # Must be above 50-day MA
    'TRAILING_STOP_ACTIVATION': 0.015,  # Activate trailing at +1.5% (BALANCED - lock profits earlier)
}
...
# Scoring System (0-10)
MIN_SIGNAL_SCORE = 7.5  # Positional: BALANCED quality (65% win rate target, selective entries)
