...
class TradingSystem:
    ...
    def __init__(self):
        ...
        # Import NSE Top 500 for scanning
        from config.nse_top_500_live import NSE_TOP_500
        
        # Use Top 500 stocks for comprehensive scanning
        self.watchlist = NSE_TOP_500.copy()
    ...
    def process_signals(self, scan_result: Dict):
        ...
        for i, signal in enumerate(swing_signals, 1):
            ...
            print(f"{i}. {symbol} - SWING (Score: {score:.1f}/10)")
        ...
        for i, signal in enumerate(positional_signals, 1):
            ...
            # IMPROVED: Filter weak setups based on signal type
            
            if signal_type == 'MEAN_REVERSION':
                # Check if mean reversion quality is valid
                ...
                if not is_valid:
                    print(f"   ❌ REJECTED - Weak mean reversion setup (need ≥50 score)")
                    print()
                    continue
                else:
                ...
                # Check if momentum quality is valid
