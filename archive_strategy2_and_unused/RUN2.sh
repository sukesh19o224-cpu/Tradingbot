...
show_menu() {
    ...
    echo "│    • Discord: ✅ Enabled (\"🎯 STRATEGY 2\" prefix)                │"
    ...
    echo "   1️⃣  Strategy 1 scans 500 stocks (~7 min)"
    echo "   2️⃣  Strategy 1 signals completion (signal file)"
    echo "   3️⃣  Strategy 2 scans 500 stocks (~7 min)"
    echo "   4️⃣  Repeat during market hours"
    echo ""
...
run_both_systems() {
    ...
    echo "   • Waits for Strategy 1 signal (file-based trigger)"
    echo "   • Scans same 500 stocks, stricter filters"
    echo "   • Discord alerts with 🎯 STRATEGY 2 prefix"
    echo ""
    echo "💡 TIP: Open dashboards to monitor:"
    echo "   • Strategy 1: http://localhost:8501"
    echo "   • Strategy 2: http://localhost:8502"
    echo ""
    echo "⏱️  Execution: Strategy 1 (7min) → Strategy 2 (7min) → Repeat (during market)"
    echo ""
...
run_both_with_dashboards() {
    ...
    echo "   → Scans 500 stocks (7 min) → Signals completion"
    echo "   → Discord: ✅ Enabled (normal alerts)"
    echo ""
    echo "🎯 Strategy 2: PID $STRATEGY2_PID | Dashboard: http://localhost:8502"
    echo "   → Waits for signal → Scans 500 stocks (7 min)"
    echo "   → Discord: ✅ Enabled (🎯 STRATEGY 2 prefix)"
    echo ""
    ...
    echo ""
    echo "⏱️  Total cycle: ~14 min (7 min Strategy 1 + 7 min Strategy 2)"
    echo ""
