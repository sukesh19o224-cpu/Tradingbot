...
class DualPortfolioStrategy2:
    ...
    def __init__(self, swing_file: str, positional_file: str, initial_capital: float = 100000):
        ...
        print(f"   🔥 Swing Portfolio: ₹{swing_capital:,.0f} (50%) - Score ≥8.3, ADX ≥32")
        print(f"   📈 Positional Portfolio: ₹{positional_capital:,.0f} (50%) - Score ≥7.5, ADX ≥27")
    ...
    def monitor_and_update_swing_positions(self) -> List[Dict]:
        """Monitor and update swing positions - returns exit info for Discord alerts"""
        try:
            ...
            if current_prices:
                exits = self.swing_portfolio.check_exits(current_prices)
                return exits
            return []
    ...
    def monitor_and_update_positional_positions(self) -> List[Dict]:
        """Monitor and update positional positions - returns exit info for Discord alerts"""
        try:
            ...
            if current_prices:
                exits = self.positional_portfolio.check_exits(current_prices)
                return exits
            return []
        except Exception as e:
